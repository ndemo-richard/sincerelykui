...
const mentalHealth =({mentalHealth, intro_post}) =>(
  ...
  <Layout>
      ...
      <div className={styles.module_header}>
        ...
        <div className={styles.main}>
          <h3 className={styles.intro_title_main}>MENTAL HEALTH </h3>
  ...
  export async function getStaticProps() {
    const intro_post = await client.fetch(`*["mental_health" in categories[]->title][0...1]| order( publishedAt desc){_id,"slug": slug.current,title,body,mainImage,publishedAt,"authorImage":author->image,"authorName":author->name}`);
    const mentalHealth = await client.fetch(`*["mental_health" in categories[]->title][1..-1]| order( publishedAt desc){_id,"slug": slug.current,title,body,mainImage,publishedAt,"authorImage":author->image,"authorName":author->name}`);
    return {
